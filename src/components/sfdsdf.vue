...
export default {
  ...
  methods: {
    async fetchProductDetails(productId) {
      try {
        const response = await axios.get(`http://127.0.0.1:8000/api/edit-product/${productId}`);
    ...
    async updateProduct() {
      ...
      try {
        ...
        const response = await axios.post(`http://127.0.0.1:8000/api/edit-store-product/${productId}`, formData, {
          headers: {
